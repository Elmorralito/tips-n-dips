...
async def export_configuration(options: ExportOptions) -> ExportResult:
    ...
    export_packages = options.get("linux_export_packages", True)
    export_desktop_settings = options.get("linux_export_desktop_settings", True)
    export_cron_jobs = options.get("linux_export_cron_jobs", True)
    ...
    with tempfile.TemporaryDirectory(prefix="migrator_staging_") as temp_dir_str:
        ...
        pkg_status = (
            await _export_system_packages(staging_dir)
            if export_packages
            else {"package_manager": detect_package_manager(), "system_packages": False, "repositories": False, "flatpak": False, "snap": False}
        )
        
        # 4. Collect Gnome Settings
        if not is_silent:
            log_info("Dumping desktop and GNOME configuration...")
        has_desktop_cfg = await _export_desktop_settings(staging_dir) if export_desktop_settings else False
        
        # 5. Collect Cron Jobs
        if not is_silent:
            log_info("Exporting crontabs...")
        has_cron = await _export_cron_jobs(staging_dir) if export_cron_jobs else False
