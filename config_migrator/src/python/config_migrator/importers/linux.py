...
from ..defaults.linux import DEFAULT_ROLLBACK_PREFIX
...
async def import_configuration(options: ImportOptions) -> ImportResult:
    ...
    restore_system_packages = options.get("linux_restore_system_packages", True)
    restore_flatpak = options.get("linux_restore_flatpak", True)
    restore_snap = options.get("linux_restore_snap", True)
    restore_desktop_settings = options.get("linux_restore_desktop_settings", True)
    restore_cron_jobs = options.get("linux_restore_cron_jobs", True)
    ...
    with tempfile.TemporaryDirectory(prefix="migrator_extract_") as temp_dir_str:
        ...
        if restore_desktop_settings and has_desktop and dconf_src.exists():
        ...
        if restore_cron_jobs and has_cron and cron_src.exists():
        ...
        # 8a. System packages
        if restore_system_packages and is_pm_compatible:
        ...
        # 8b. Flatpaks
        missing_flatpaks = await _get_missing_flatpaks(staging_dir) if restore_flatpak else []
        if missing_flatpaks:
            if not is_silent:
        ...
        # 8c. Snaps
        missing_snaps = await _get_missing_snaps(staging_dir) if restore_snap else []
        if missing_snaps:
            if not is_silent:
